// src/features/candidates/candidateMock.ts
// Mock data store for candidates — used while backend is not ready
// Replace all usages with real API calls when Spring Boot is running

import { Candidate, RecruitmentStage } from './candidateTypes';

// In-memory store — persisted to sessionStorage so it survives page refresh
...
export function mockCreateCandidate(values: Omit<Candidate, 'id' | 'stage' | 'createdAt' | 'updatedAt'>): Candidate {
